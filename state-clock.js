"use strict";
{
  ...
  function render_old(textrack, where, memory = {id_stack: [],roots:[]}) {
      ...
      switch( node.nodeType ) {
        case Node.ELEMENT_NODE: {
          ...
          try {
            ...
            const renderedAgainComponentHtml = render(componentHtml,null,memory);
            if ( componentHtml !== renderedAgainComponentHtml ) {
              componentHtml = renderedAgainComponentHtml; 
            }
            if ( !! component ) {
              const currentComponent = component;
              const cnode = df(componentHtml);
              const rid = cnode.querySelector('[data-ractid]').dataset.ractid;
              memory.roots.push(() => {
                const root = document.querySelector(`[data-ractid="${rid}"]`);
                currentComponent.root = root;
                currentComponent.componentDidMount();
        ...
        default: {
          if ( !! node.nodeValue ) {
            const renderedAgainComponentHtml = render(node.nodeValue,null,memory);
